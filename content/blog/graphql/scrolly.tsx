  ...
  return (
    <div className="md:hidden">
      {blocks.map((step, i) => (
        <section key={i}>
          {step.children}
          <Code codeblock={step.child} />
          <Code codeblock={step.parent} />
          {/* <Preview i={i} /> */}
          {step.after?.children}
        </section>
      ))}
...
function Sticker({ step, index }: { step: Step; index: number }) {
  return (
    <div className="flex gap-2" style={{ height: stickerHeight }}>
      <Code className="my-0 flex flex-col flex-1" codeblock={step.parent} />
      <div className="flex flex-col flex-1 gap-2">
        <Code className="my-0 flex flex-col flex-1" codeblock={step.child} />
        {/* <Preview i={index} /> */}
      </div>
    </div>
  )
